...
def to_temperature(raw_value):
    """Convert raw 16-bit thermal data to Celsius."""
    return raw_value / 64.0 - 273.15


def save_thermal_vtk(thermal_data_16, filepath):
    """Save 192x256 thermal data as VTK structured grid with temperature field."""
    temp_c = to_temperature(thermal_data_16.astype(np.float64))
    ny, nx = temp_c.shape  # 192, 256

    with open(filepath, 'w') as f:
        f.write("# vtk DataFile Version 3.0\n")
        f.write("P2Pro Thermal Data\n")
        f.write("ASCII\n")
        ...
        for j in range(ny):
            for i in range(nx):
                f.write(f"{i} {ny - 1 - j} 0\n")
        f.write(f"\nPOINT_DATA {nx * ny}\n")
        f.write("SCALARS Temperature_C float 1\n")
        f.write("LOOKUP_TABLE default\n")
        for j in range(ny):
            for i in range(nx):
                f.write(f"{temp_c[j, i]:.2f}\n")

    log.info(f"VTK saved: {filepath}")
    tmin, tmax = np.min(temp_c), np.max(temp_c)
    log.info(f"  Temperature range: {tmin:.1f} ~ {tmax:.1f} °C")
...
class Video:
    ...
    def open(self, cam_cmd=None, camera_id: Union[int, str] = -1):
        if camera_id == -1:
        ...
        if cap_res != P2Pro_resolution:
            raise IndexError(
                f"Resolution of camera id {camera_id} doesn't match. It's probably not a P2 Pro. (Got: {cap_res[0]}x{cap_res[1]}@{cap_fps})")
        log.info(f"Camera opened: {cap_res[0]}x{cap_res[1]} @ {cap_fps} FPS")
        ...
        while True:
            ...
            # --- Temperature overlay on display ---
            temp_c = to_temperature(thermal_picture_16.astype(np.float64))
            t_min, t_max = np.min(temp_c), np.max(temp_c)
            t_center = temp_c[thermal_picture_16.shape[0] // 2, thermal_picture_16.shape[1] // 2]

            # Normalize and colormap for display
            if t_max - t_min > 0:
                norm = (temp_c - t_min) / (t_max - t_min)
            else:
                norm = np.zeros_like(temp_c)
            display_scale = 3
            h_disp = int(norm.shape[0] * display_scale)
            w_disp = int(norm.shape[1] * display_scale)
            show_img = cv2.resize(norm, (w_disp, h_disp), interpolation=cv2.INTER_CUBIC)
            show_img = (show_img * 255).astype(np.uint8)
            show_img = cv2.applyColorMap(show_img, cv2.COLORMAP_INFERNO)

            # Draw center crosshair
            cx, cy = w_disp // 2, h_disp // 2
            cv2.drawMarker(show_img, (cx, cy), (255, 255, 255), cv2.MARKER_CROSS, 20, 1)

            # Temperature text overlay
            info_lines = [
                f"Center: {t_center:.1f} C",
                f"Min: {t_min:.1f} C  Max: {t_max:.1f} C",
                f"Range: {t_min:.1f} ~ {t_max:.1f} C",
            ]
            for idx, line in enumerate(info_lines):
                y_pos = 25 + idx * 25
                cv2.putText(show_img, line, (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 3)
                cv2.putText(show_img, line, (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)

            cv2.imshow('P2Pro Thermal', show_img)
            key = cv2.waitKey(1)
            if key & 0xFF == ord('q'):
                break
            elif key == 13:  # ENTER key -> save VTK
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                vtk_dir = os.path.join(os.path.expanduser("~"), "P2Pro_VTK")
                os.makedirs(vtk_dir, exist_ok=True)
                vtk_path = os.path.join(vtk_dir, f"thermal_{timestamp}.vtk")
                save_thermal_vtk(thermal_picture_16, vtk_path)
                png_path = os.path.join(vtk_dir, f"thermal_{timestamp}.png")
                cv2.imwrite(png_path, show_img)
                log.info(f"Screenshot saved: {png_path}")
            elif cam_cmd and key != -1:
                try:
                    if key & 0xFF == ord('s'):
                        cam_cmd.shutter_actuate()
                    elif key & 0xFF == ord('d'):
                        cam_cmd.get_shutter_state()
                    elif key & 0xFF == ord('b'):
                        cam_cmd.shutter_background()
                    elif key & 0xFF == ord('l'):
                        cam_cmd.gain_set_low()
                        log.info("Switched to LOW gain (high temp)")
                    elif key & 0xFF == ord('h'):
                        cam_cmd.gain_set_high()
                        log.info("Switched to HIGH gain (normal)")
                except Exception as e:
                    log.warning(f"USB command failed: {e}")
